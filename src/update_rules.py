...
def s_mse_single_ferro_antiferro(conductances, incidence_matrix, Q_inputs, Q_outputs, inputs, true_output):
    ''' Compute the mean square error. '''
    # input_vector =  self.circuit_input(inputs, self.indices_inputs, self.current_bool)
    V = Circuit.s_solve_ferro_antiferro(conductances, incidence_matrix, Q_inputs, inputs)
    predicted_output = Q_outputs.T.dot(V)
    return 0.5*jnp.mean((predicted_output - true_output)**2)

@jax.jit
def s_mse_ferro_antiferro(conductances, incidence_matrix, Q_inputs, Q_outputs, inputs, true_outputs):
    ''' Compute the mean square error for batches of inputs and outputs. '''
    batch_mse = jax.vmap(s_mse_single_ferro_antiferro, in_axes=(None, None, None, None, 0, 0))
    mse_values = batch_mse(conductances, incidence_matrix, Q_inputs, Q_outputs, inputs, true_outputs)
    return jnp.mean(mse_values) 


@jax.jit
def s_grad_mse_ferro_antiferro(conductances, incidence_matrix, Q_inputs, Q_outputs, inputs, true_output):
    ''' Compute the gradient of the mean square error. '''
    grad_func = jax.grad(s_mse_ferro_antiferro, argnums=0)
    return grad_func(conductances, incidence_matrix, Q_inputs, Q_outputs, inputs, true_output)
...
def gd_update_ferro_antiferro(
    ...
    circuit_batch = (circuit_input_batch(circuit.jax, batch[0], circuit.indices_inputs, circuit.current_bool, circuit.n), batch[1])
    inputs, targets = circuit_batch
    grad = s_grad_mse_ferro_antiferro(
        circuit.conductances,
        circuit.incidence_matrix,
        circuit.Q_inputs,
        circuit.Q_outputs,
...
def cl_update_ferro_antiferro(
    ...
    # free phase
    states_free = circuit.solve_batch_ferro_antiferro(circuit.Q_inputs,inputs)
    y_free = np.dot(states_free, circuit.Q_outputs.toarray())
    # clamped phase
    nudge = y_free + eta * (targets - y_free)
    clamped_inputs = np.concatenate((inputs, nudge), axis=1)
    ...
    states_clamped = circuit.solve_batch_ferro_antiferro(circuit.Q_clamped, clamped_inputs)
